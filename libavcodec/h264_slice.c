    ...
    while (v->poc.frame_num != v->poc.prev_frame_num && !h->first_field &&
        ...
        if (v->short_ref_count) {
            ...
            if (prev &&
                 ...
                 * their INT_MAX init value breaks their reference
                 * ordering. */
                v->short_ref[0]->field_poc[0] = v->short_ref[0]->poc;
                v->short_ref[0]->field_poc[1] = v->short_ref[0]->poc;
                ...
                av_refstruct_replace(&v->short_ref[0]->hwaccel_picture_private,
